...
public class HealthInsurance {
    ...
    public static ArrayList<HealthInsurance> healthInsurance = new ArrayList<>();

    public HealthInsurance(int precio, String tipo, String userid, String ibanPaga) {
        this.precio = precio;
        this.tipo = tipo;
        this.userid = userid;
    ...
    public void basico() {
        ...
        if (bankAccountEncontrado instanceof DebitAccount) {
            ...
            if (debit.balance >= precio2) {
                ...
                HealthInsurance coche = new HealthInsurance(720, "basico", userEncontrado.userid, debit.IBAN);
                healthInsurance.add(coche);
            } else {
            ...
            if (credit.balance >= precio2) {
                ...
                HealthInsurance life = new HealthInsurance(720, "basico", userEncontrado.userid, credit.IBAN);
                healthInsurance.add(life);
            } else {
    ...
    public void moderado() {
        ...
        if (bankAccountEncontrado instanceof DebitAccount) {
            ...
            if (debit.balance >= precio2) {
                debit.balance = -precio2;
                HealthInsurance coche = new HealthInsurance(1200, "basico", userEncontrado.userid, debit.IBAN);
                healthInsurance.add(coche);
            } else {
                System.out.println("Dinero insuficiente.");
            }
            ...
            if (credit.balance >= precio2) {
                credit.balance = - precio2;
                HealthInsurance life = new HealthInsurance(1200, "basico", userEncontrado.userid, credit.IBAN);
                healthInsurance.add(life);
            } else {
                double available = credit.balance - precio2;
                if (available < 0) {
    ...
    public void premium() {
        ...
        if (bankAccountEncontrado instanceof DebitAccount) {
            ...
            if (debit.balance >= precio2) {
                debit.balance = -precio2;
                HealthInsurance coche = new HealthInsurance(2000, "basico", userEncontrado.userid, debit.IBAN);
                healthInsurance.add(coche);
            } else {
                System.out.println("Dinero insuficiente.");
            }
            ...
            if (credit.balance >= precio2) {
                credit.balance = - precio2;
                HealthInsurance life = new HealthInsurance(160, "basico", userEncontrado.userid, credit.IBAN);
                healthInsurance.add(life);
            } else {
                double available = credit.balance - precio2;
                if (available < 0) {
                    ...
                    credit.balance = 0;
                    System.out.println("Tienes " + credit.deudaCredit + " de deuda.");
                    HealthInsurance life = new HealthInsurance(2000, "basico", userEncontrado.userid, credit.IBAN);
                    healthInsurance.add(life);
                    return;
                }
                credit.balance -= precio2;
    ...
    static void comprobarYCobrarHealth(HealthInsurance health) {

        if (health.getFechaSeguro().plusYears(1).isBefore(LocalDate.now())) {

            BankAccount cuenta = buscarCuenta(health.getIbanPaga());

            if (cuenta != null) {

                int precio = health.getPrecio();
                ...
                if (cuenta instanceof DebitAccount) {
                    ...
                    if (debit.balance >= precio) {
                        ...
                        health.setFechaSeguro(LocalDate.now());
